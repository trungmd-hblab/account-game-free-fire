...
function FlipCardForm(props) {
  ...
  const { data: dataLuckyWheel, isLoading } = useGetLuckyWheelById(
    id,
    mode === "edit" && !!id
  );
  ...
  useEffect(() => {
    if (dataLuckyWheel) {
      ...
      dataLuckyWheel?.result?.thumbnailUrl &&
        ...
        let newpreviewPrizesImage = dataLuckyWheel?.result?.prizes?.map((prize) => {
  ...
  const handleImageChangePrize = async (file, index) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        let newPreviewPrizesImage = previewPrizesImage;
    ...
    try {
      ...
      toast.error(error?.response?.data?.message ? error?.response?.data?.message : "Tải ảnh lỗi. Hãy thử lại", {
        position: "bottom-center",
        autoClose: 1000,
        closeOnClick: true,
        pauseOnHover: true,
      ...
      >
        <div className="lg:flex justify-between gap-10">
          <div className="flex-[2] flex flex-col gap-1 mb-2">
            <TextInput
              label="Tên game"
              {...register("name", { required: "Tên game bắt buộc" })}
                ...
                <div className="flex gap-2">
                  ...
                  <TextInput
                    ...
                    {...register(`prizes.${index}.name`, {
                      required: "Tên ô thưởng bắt buộc",
                    })}
                    error={
