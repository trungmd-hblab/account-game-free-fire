"use client";
import { useFetchPhoneCardHistory } from "@/api/client/history_client";
...
function HistoryByType() {
    ...
    const columns = ['Nhà mạng', 'Sô thẻ', 'Số seri', 'Giá trị thẻ', 'Trạng thái', 'Thời gian yêu cầu'];

    const { data: response, isLoading } = useFetchPhoneCardHistory({
        page: currentPage,
        limit: limitView,
        ...
        if (data?.length) {
            return data.map((item) => (
                <Table.Tr key={item._id}>
                    <Table.Td>{item.carrier}</Table.Td>
                    <Table.Td>{item.cardNumber}</Table.Td>
